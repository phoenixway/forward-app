...
const ICON_CONFIGS: IconMarkerConfig[] = [
  { icon: '❗', markers: ['#critical', '! ', '!'], className: 'text-red-500 mr-1', isSuffixOnly: true },
  { icon: '☀️', markers: ['#day', '+ '], className: 'text-yellow-500 mr-1' },
  { icon: '🗓️', markers: ['#week', '++ '], className: 'text-blue-500 mr-1' },
  { icon: '🎯', markers: ['#middle-term', '+++ '], className: 'text-green-500 mr-1' },
  { icon: '🔭', markers: ['#long-term', '~ ', '~'], className: 'text-purple-500 mr-1', isSuffixOnly: true },
];
...
interface GoalTextRendererProps {
  ...
  onTagClick?: (filterTerm: string) => void; // Новий колбек для кліку на тег
}
...
const GoalTextRenderer: React.FC<GoalTextRendererProps> = ({
  ...
  stripFields = false,
  obsidianVaultName,
  onTagClick, // Отримуємо новий проп
}) => {
  const handleWikiLinkClick = useCallback(async (event: React.MouseEvent<HTMLAnchorElement>, linkContent: string) => {
    ...
    if (!obsidianVaultName) {
      const errorMessage = 'Назва Obsidian Vault не налаштована. Будь ласка, перевірте налаштування.';
    ...
    if (window.electronAPI && typeof window.electronAPI.openExternal === 'function') {
      console.log('[GoalTextRenderer] window.electronAPI.openExternal is a function. Calling it...');
      try {
        // @tys-expect-error
        // @ts-ignore
        const result = await window.electronAPI.openExternal(obsidianUrl);
        if (result && result.success) {
          console.log(`[GoalTextRenderer] Successfully initiated opening of ${obsidianUrl} via preload.`);
        } else if (result && !result.success) {
          const preloadErrorMessage = result.error || 'Невідома помилка в preload-скрипті.';
          console.error(`[GoalTextRenderer] Preload script failed to open link: ${preloadErrorMessage}`);
          alert(`Не вдалося відкрити посилання: ${preloadErrorMessage}`);
        } else if (typeof result === 'undefined') {
          console.log(`[GoalTextRenderer] Link opening initiated (void return from preload for ${obsidianUrl}).`);
        } else {
          ...
          alert(`Не вдалося відкрити посилання: ${unexpectedResponseMessage}`);
        }
      } catch (error) {
        const callErrorMessage = (error instanceof Error) ? error.message : String(error);
        console.error('[GoalTextRenderer] Error calling window.electronAPI.openExternal:', error);
        alert(`Помилка під час спроби відкрити посилання: ${callErrorMessage}`);
      }
    } else {
      const apiUnavailableMessage = 'window.electronAPI or window.electronAPI.openExternal is NOT available.';
      console.error(`[GoalTextRenderer] ${apiUnavailableMessage}`);
      alert('API для відкриття зовнішніх посилань недоступне.');
    }
  }, [obsidianVaultName]);

  const parseTextAndExtractData = useCallback((inputText: string) => {
    // ... (код parseTextAndExtractData без змін, як у попередній версії) ...
    ...
    for (const config of ICON_CONFIGS) {
      for (const marker of config.markers) {
        ...
        if (config.isSuffixOnly && (marker === '!' || marker === '~') && currentText.endsWith(marker)) {
          if (currentText.length === marker.length || (currentText.length > marker.length && currentText[currentText.length - marker.length - 1] === ' ')) {
             markerFound = true;
          }
        ...
        if (markerFound) {
          if (!iconsToRender.find(iconConf => iconConf.icon === config.icon)) {
            iconsToRender.push(config);
          }
          if (config.isSuffixOnly && (marker === '!' || marker === '~') && currentText.endsWith(marker) && !marker.endsWith(' ')) {
            currentText = currentText.substring(0, currentText.length - marker.length).trim();
          } else {
    ...
    iconsToRender.sort((a, b) => ICON_CONFIGS.indexOf(a) - ICON_CONFIGS.indexOf(b));
    ...
    if (stripFields) {
      ...
      while ((matchWhile = fieldRegex.exec(tempTextForFieldStripping)) !== null) {
          fields.push({ name: matchWhile[1].trim(), value: matchWhile[2].trim() });
      }
      currentText = currentText.replace(fieldRegex, '').trim();
    }
    
    currentText = currentText.replace(/\s\s+/g, ' ').trim();
    return { mainText: currentText, fields, icons: iconsToRender };
  }, [stripFields]);
  ...
  const renderStyledText = (textToRender: string): JSX.Element => {
    const combinedRegex = /(\[\[([^|\]]+)(?:\|([^\]]+))?\]\])|(@([a-zA-Z0-9_а-яА-ЯіІїЇєЄ'-]+))|((?:\B|^)#([a-zA-Z0-9_а-яА-ЯіІїЇєЄ'-]+)\b)/g;
    // @ghts-expect-error
    // @ts-ignore
    ...
    while ((match = combinedRegex.exec(textToRender)) !== null) {
      ...
      if (match[1]) { // Вікі-посилання
        ...
        parts.push(
          <a
            ...
            onClick={(e: React.MouseEvent<HTMLAnchorElement>) => handleWikiLinkClick(e, linkTarget)}
      ...
      else if (match[4]) { // @-теги / @мета_цілі
        const atSign = '@';
    ...
    return <>{parts.map((part, index) => <React.Fragment key={`part-${index}`}>{part}</React.Fragment>)}</>;
  };
  ...
  return (
    <div className="goal-text-renderer break-words flex items-center">
      {icons.length > 0 && (
        <span className="icons-container mr-1 flex">
