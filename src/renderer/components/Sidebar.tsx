          ...
          >
            {(provided, snapshot) => (
              <div
                ...
                className={`group rounded-md transition-colors duration-100
                  ${
                    snapshot.isDraggingOver
                      ? "bg-green-100 dark:bg-green-700/50 ring-1 ring-green-500"
                ...
                {editingListId === list.id ? (
                  <div className="p-2 border border-blue-400 dark:border-blue-600 rounded-md bg-white dark:bg-slate-800 shadow">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setEditingListDescription(e.target.value)
                      }
                      placeholder="Опис списку (опціонально)"
                      rows={2}
